...

        {
            ...
            for(index=0; index<5; index++) {
                ...
                switch(index) {
                    ...
                    default: printf("Tx unKnown\n");          break;
                }
#endif
                write(devfd, ublox_conf[index].payload, ublox_conf[index].size);
#if 0
                for(j=0; j<ublox_conf[index].size; j++)
                    printf("0x%02x ", ublox_conf[j].payload[j]);
                printf("\n");
#endif
                printf("Rx : ");

                while(1) {
                    read(devfd, &rxbuf[pos], 1);

                    if(rxbuf[pos] == r_cfg[pos]) {
                        printf("%02x ", rxbuf[pos]);
                        pos++;
                    }

                    if(pos >= 3) {
                        read(devfd, &rxbuf[pos], 7);
